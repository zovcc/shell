...
export function stack_move_left(forest: Forest, stack: NodeStack, entity: Entity): boolean {
    const container = forest.stacks.get(stack.idx);
    ...
    for (const cmp of stack.entities) {
        if (Ecs.entity_eq(cmp, entity)) {
            if (idx === 0) {
                ...
                container.windows[idx].button.destroy();
                container.windows.splice(idx, 1);
                moved = false;
            } else {
...
export function stack_move_right(forest: Forest, stack: NodeStack, entity: Entity): boolean {
    const container = forest.stacks.get(stack.idx);
    if (!container) return false;
...
export function stack_remove(forest: Forest, stack: NodeStack, entity: Entity): void {
    const container = forest.stacks.get(stack.idx);
    if (!container) return;
